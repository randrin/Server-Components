package com.nbp.bear.components.constant;

public class NbpResponse {

    public static final String NBP_USER_CREATED = "User created successfully.";
    public static final String NBP_USER_USERNAME_EXISTS = "User already exists with username.";
    public static final String NBP_USER_EMAIL_EXISTS = "User already exists with email.";
    public static final String NBP_USER_LOGGED = "User logged successfully.";
    public static final String NBP_USER_LOGOUT = "User logout successfully.";
    public static final String NBP_USER_DELETED = "User deleted successfully.";
    public static final String NBP_USER_UPDATED = "User updated successfully.";
    public static final String NBP_USER_PASSWORD_GENERATED = "User Password generated successfully.";
    public static final String NBP_USER_PASSWORD_CHANGED = "User Password changed successfully.";
    public static final String NBP_USER_ACTIVATED = "User activated successfully.";
    public static final String NBP_USER_DISABLED = "User disabled successfully.";
    public static final String NBP_USER_UPDATE_PROFIL = "User Profil update successfully.";

    // ERRORS
    public static final String NBP_USER_UPDATE_PROFIL_NOT_FOUND = "User Profil can't be update.";
    public static final String NBP_USER_ERROR_LOGIN = "Bad Credentials Username/Password incorrect.";
    public static final String NBP_USER_ERROR_NOT_FOUND = "User not found.";
    public static final String NBP_USER_ERROR_PASSWORD = "User password isn't correct. Try again !!";
    public static final String NBP_USER_ERROR_PASSWORD_NOT_MATCH = "User old password isn't correct. Try again !!";
    public static final String NBP_USER_ERROR_DISABLE = "Your account isn't active. Contact your administration department for more information.";

}
